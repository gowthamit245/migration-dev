...
(function ($, window) {
    ...
    PickList.prototype.initHtml = function () {
        ...
        if (this.options.label !== false) {
            available.prepend($('<p class="mx-3 my-3 text-center">').text(this.options.label.content[0]));
            selected.prepend($('<p class="mx-3 my-3 text-center">').text(this.options.label.content[1]));
        }
    ...
    PickList.prototype.initButtons = function () {
        ...
        $.each(this.options.buttons, function (i, v) {
            var button = $('<button class="" style="background-color: #2A1B60">').attr('type', 'button');
    ...
    PickList.prototype.initSelect = function () {
        var container = $('<div class="dropdown_select bg-danger list-group pt-1 pb-5 text-light">').addClass('select');

        $('<select id="dropdown_select" class="p-3 dropdown_select_list list-group-item">').prop('multiple', true).attr('size', this.options.select.size).appendTo(container);
        return container;
    };
    ...
    PickList.prototype.initData = function () {
        ...
        $.each(this.available, function (i, v) {
            $('<option class=" py-1 text-dark">').val(v.id).text(v.label).data('item', v).appendTo(available);
        });

        $.each(this.selected, function (i, v) {
            $('<option class="text-dark">').val(v.id).text(v.label).data('item', v).appendTo(selected);
        });
    };
